  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Get upcoming assignments (with due dates) for filtering today/tomorrow
        const upcomingAssignments = await db.getAllUpcomingAssignments();
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-800">Tasks Due Today</h2>
          </div>
          <div className="p-6">
            {stats.tasksListToday.length === 0 ? (
              <div className="text-center py-8">
                <svg className="w-16 h-16 text-gray-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <p className="text-gray-500 text-lg">No tasks due today! 🎉</p>
                <p className="text-gray-400 text-sm mt-2">Enjoy your day or get ahead on tomorrow&apos;s work.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {stats.tasksListToday.map((task) => (
